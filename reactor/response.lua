...
function ResponseModel:new(obj)
	obj = obj or {}
...
function ResponseModel:tune(x, actual, predicted)
	...
	if _G.log then
		_G.log.info("SM> actual: "..actual.."; predicted: "..predicted.."; error: "..(actual - predicted))
...
function ResponseModel:load(filename)
	if filename then
...
function ResponseModel:save(filename)
	if filename then
		self.file = io.open(filename, 'r+b')
